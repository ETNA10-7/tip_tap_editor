...
export const getCurrentUser = query(async (ctx) => {
  ...
  if (!userId) {
    ...
    console.log(
      "[getCurrentUser] This means ConvexAuthProvider is not sending session token with the query",
    );
    console.log(
      "[getCurrentUser] Check if session token is stored in localStorage after page reload",
    );
    return null;
  }
  ...
  if (!user) {
    ...
    console.error(
      "[getCurrentUser] ❌ CRITICAL ERROR: User authenticated but document not found!",
    );
    console.error("[getCurrentUser] userId:", userId);
    console.error(
      "[getCurrentUser] This means auth system is broken - user exists in authAccounts but not in users table",
    );
    throw new Error(
      "User authenticated but user document not found in database. This is a critical error.",
    );
  }
...
export const updateProfile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!userId) {
      throw new Error(
        "Not authenticated. Please sign in to update your profile.",
      );
    }
...
export const getByUsername = query({
  ...
  handler: async (ctx, args) => {
    ...
    const userByUsername = await ctx.db
      .query("users")
      .withIndex("username", (q) => q.eq("username", searchUsername))
      .first();
...
export const updateUsername = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!userId) {
      throw new Error(
        "Not authenticated. Please sign in to update your username.",
      );
    }
