...
export default function CreatePage() {
  ...
  useEffect(() => {
    if (!isLoading && !isAuthenticated) {
      console.log(
        "[CreatePage] ❌ User not authenticated - opening auth modal",
      );
      openModal("login");
    ...
    <>
      ...
      <div className="space-y-6 relative z-0">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <label
              className={`text-sm font-medium ${isLightMode ? "text-black" : "text-slate-300"}`}
            >
            ...
            />
            <p
              className={`text-xs ${isLightMode ? "text-black" : "text-slate-400"}`}
            >
        ...
        {error ? (
          <p
            className={`text-sm ${isLightMode ? "text-red-600" : "text-red-400"}`}
          >
            {error}
          </p>
        ) : null}
