...
export default function ProfilePage() {
  ...
  // Get user's posts
  const posts = useQuery(api.posts.listByUser, isAuthenticated ? {} : "skip");
              ...
              <div className="flex-1 space-y-4 min-w-0">
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                  <div className="space-y-1 min-w-0">
                    <h1
                      className={`text-3xl font-bold truncate ${isLightMode ? "text-black" : "text-slate-900"}`}
                    >
                      {user.name || "User"}
                    </h1>
                    {user.email && (
                      <div
                        className={`flex items-center gap-2 ${isLightMode ? "text-black" : "text-muted-foreground"}`}
                      >
                ...
                {user.bio ? (
                  <div className="pt-2">
                    <p
                      className={`leading-relaxed ${isLightMode ? "text-black" : "text-slate-700"}`}
                    >
                      {user.bio}
                    </p>
                  </div>
                  ...
                  <div className="pt-2">
                    <p
                      className={`italic ${isLightMode ? "text-black" : "text-muted-foreground"}`}
                    >
                      No bio yet.{" "}
                      <Link
                        href="/profile/edit"
              ...
              <div className="rounded-xl border-2 border-dashed border-slate-200 bg-slate-50/50 p-12 text-center">
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <p
                      className={`font-medium ${isLightMode ? "text-black" : "text-slate-700"}`}
                    >
                      {postView === "published"
