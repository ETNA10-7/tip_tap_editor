  ...
  const convex = useMemo(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      const authKeys = allKeys.filter(
        (k) =>
          k.includes("convexAuthJWT") || k.includes("convexAuthRefreshToken"),
      );

      if (authKeys.length > 0) {
        console.log(
          "[ConvexClientProvider] ✅ Auth tokens found in localStorage:",
          authKeys,
        );
        authKeys.forEach((key) => {
          const token = localStorage.getItem(key);
          console.log(
            `[ConvexClientProvider] Token ${key}:`,
            token ? `${token.substring(0, 20)}...` : "null",
          );
        });
      } else {
        console.log(
          "[ConvexClientProvider] ⚠️ No auth tokens found in localStorage",
        );
      }
    }
